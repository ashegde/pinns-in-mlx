...
from configs.config_schema import LogConfig
...
class Logger:
    ...
    def log(self, text: str):
        with open(f"{self.config.output_dir}/{self.config.log_file}", "a") as file:
            file.write(text+"\n")
    ...
    def checkpoint(self, model: nn.Module, file_name: str):
        ...
        model.save_weights(file=f"{self.config.output_dir}/{file_name}")

    def load_log(self) -> list[str]:
        log = []
        with open(f"{self.config.output_dir}/{self.config.log_file}", "r") as file:
            for line in file:
                log.append(line)
        return log